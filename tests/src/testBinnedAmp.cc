...
int main( int argc, char** argv )
{
  ...
  std::vector< Parameter > pars { mu, sigma };
  std::vector< Coef      > coefs{ Coef( mu, sigma ) };
  Binning binning;
  BinnedAmplitude amp( pars, pars, coefs, binning );
  Decay3BodyBinned cb( x, x, x, amp, coefs[0] * 1, PhaseSpace( 4.0, 3.0, 2.0, 1.0 ) );
