...
int main( int argc, char** argv )
{
  ...
  Parameter beta ( "beta" ,  2.0  ,   0.003 );
  Parameter m    ( "m"    ,  2.1  ,   0.003 );

  // Definition of the pdf.
  DoubleCrystalBall cb( x, mu, sigma, alpha, n, beta, m );

  if ( std::string( argv[ 1 ] ) == "gen" )
  {
    // Open the output file.
    std::ofstream output( "data/doublecrystalball.dat" );
  ...
  if ( std::string( argv[ 1 ] ) == "fit" )
  {
    ...
    // Read the data.
    readData( "data/doublecrystalball.dat", data );
