...
int main( int argc, char** argv )
{
  ...
  Parameter gamma( "gamma",  2.0  , 0.01  );

  // Definition of the pdf.
  Exponential expo( x, gamma );

  expo.setLowerLimit( -1.0 );

  if ( std::string( argv[ 1 ] ) == "gen" )
  {
    // Open the output file.
    std::ofstream output( "data/exponential.dat" );
  ...
  if ( std::string( argv[ 1 ] ) == "fit" )
  {
    ...
    // Read the data.
    readData( "data/exponential.dat", data );
